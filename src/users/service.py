...
class UserService(BaseService[User, UserCreate, UserUpdate]):
    ...
    async def get_purchases_with_filters(self,user_id:int | None = None,
        ...
        .where(Purchase.student_id == user_id if user_id else True,
                ...
                User.phone_number == ph_no if ph_no else True,
                 ...
                 Admission.id == admission_id if admission_id else True,or_(PurchaseInstallment.is_deleted == False,
                                                                                  PurchaseInstallment.is_deleted == None))
    ...
    async def get_purchases_with_prod_admission_data(self,user_id:int | None = None,limit: int = 100, offset: int = 0,db_session: AsyncSession | None = None):
        ...
        .join(Admission,Admission.id == Purchase.admission_id)
        .outerjoin(sub_query, sub_query.c.purchase_id == Purchase.id)
        .where(Purchase.student_id == user_id if user_id else True ).order_by(Purchase.id.desc()).offset(offset).limit(limit))
        result = await session.execute(query)
        purchases = result.all()
        return purchases
    ...
    async def txs_by_date_window(self,start_date:date,end_date:date,branch_ids:list[int] | None = None, include_incomplete_txs: bool | None = None,is_online_branch: bool | None = None,limit:int | None = None,plan_name:str | None = None,offset:int | None = None,db_session:AsyncSession | None = None):
        ...
        .where(
            ...
            or_(PurchaseInstallment.is_deleted == False,PurchaseInstallment.is_deleted == None),
            and_(
                cast(timestamp_ist, Date) >= start_date if start_date else True,
                cast(timestamp_ist, Date) <= end_date if end_date else True
                ))
    ...
    async def pending_purchases(self,limit:int | None = None,offset:int | None = None,branch_ids:list[int] | None = None,db_session:AsyncSession | None = None):
        session = db_session
    ...
    async def get_active_purchases_for_student(self,student_id:int,db_session:AsyncSession| None = None):
        session = db_session
        now = datetime.now()
...
class TransactionService(
    ...
    async def get_subs_txs(self,start_date:date| None = None,end_date:date| None = None,plan_id:int| None = None, plan_name:str| None = None,
        ...
        query = (
            ...
            .join(User, User.id == Subscription.user_id)
            .where(cast(timestamp_ist, Date) >= start_date if start_date else True,
                cast(timestamp_ist, Date) <= end_date if end_date else True,
