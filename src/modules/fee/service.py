...
from sqlalchemy import String, case,  cast, literal_column, select,func,and_,or_
...
class PurchaseService(BaseService[Purchase, PurchaseCreate, PurchaseUpdate]):
    ...
    async def get_all_fees( self,from_date: date,
        till_date: date | None = None , branch_id:int | None = None, offering_id:int | None = None, batch_id:int | None = None, db_session: AsyncSession | None = None
    ):
        session = db_session 
        filters = []
        ...
        unpaid_pur_inst_query = (
            ...
            .where(and_(*filters, PurchaseInstallment.installment_date.between(start_date_with_tz, end_date_with_tz),
              ...
              PurchaseInstallment.is_deleted == None), PurchaseInstallment.installment_status !=  "COMPLETED", Purchase.purchase_status != "CANCELLED"))
    ...
    till_date: date | None = None , branch_id:int | None = None, db_session: AsyncSession | None = None
        ...
        unpaid_pur_inst_query = (select(func.sum(PurchaseInstallment.installment_amount).label("unpaid_pur_installment"),Product.offering_id.label("offering_id"))
                                ...
                                .join(Product, Product.id == Purchase.product_id)
                               .where(and_(*filters,PurchaseInstallment.installment_date.between(start_date_with_tz, end_date_with_tz)
                                           ,PurchaseInstallment.installment_status !=  "COMPLETED",Purchase.purchase_status != "CANCELLED",or_(PurchaseInstallment.is_deleted == False,
                                                                                  PurchaseInstallment.is_deleted == None)))
                                .group_by(Product.offering_id))
    ...
    async def get_day_report(self,from_date: date,
        till_date: date,plan_name: str | None = None,
        ...
        .where(
            ...
            or_(PurchaseInstallment.is_deleted == False,PurchaseInstallment.is_deleted == None),
            and_(
                cast(timestamp_ist, Date) >= from_date if from_date else True,
                cast(timestamp_ist, Date) <= till_date if till_date else True
                ),
            # or_(
            #     Purchase.id.isnot(None),
            #     PurchaseInstallment.id.isnot(None),
    ...
    async def get_walkins_by_filter(self,by_date:bool, by_reason: bool, by_competitor: bool,
        ...
        filters = [Walkin.created_at.between(from_date, till_date)] if from_date and till_date else []
        ...
        if by_counsellor:
            query = query.add_columns(counsellor_expr.label("counsellor_name"))
            group_by_columns.append(counsellor_expr)
        if by_probability:
    ...
    async def get_admissions_by_filter(self,by_date:bool,by_branch: bool,by_batch:bool, by_probability: bool, by_counsellor:bool ,
        ...
        filters = [Admission.admission_date.between(from_date, till_date)] if from_date and till_date else []
        ...
        if by_branch:
            query = query.add_columns(Admission.branch_id.label("branch_id"))
        ...
        if by_probability:
            probability_expr = cast(Walkin.counselling.op("->>")("probability"), String)
            query = (query.add_columns((probability_expr).label("probability"))
                     .select_from(Admission).join(Walkin,Walkin.admission_id == Admission.id))
            group_by_columns.append(probability_expr)
        if by_counsellor:
            counsellor_expr = cast(Walkin.counselling.op("->>")("counsellor_name"), String)
            query = (query.add_columns((counsellor_expr).label("counsellor"))
                     .select_from(Admission).join(Walkin,Walkin.admission_id == Admission.id))
            group_by_columns.append(counsellor_expr)
        if by_district:
            query = (query.add_columns((district_expr).label("district"))
        ...
        for row in rows:
            ...
            if by_branch:
                entry["branch_id"] = row.get("branch_id", "Unknown")  # Replace with actual branch name lookup
            if by_offering:
    ...
    async def get_avg_days_walkin_to_admission(self,branch_id: int | None = None,batch_id:int | None = None,
        ...
        if from_date and till_date:
            filters.append(Walkin.created_at.between(from_date, till_date))
        ...
        subquery = (
            select(
                (func.extract("epoch", func.age(Admission.admission_date, Walkin.created_at)) / 86400).label("days_diff"),
                # Walkin.admission_id,
                extract("month", Walkin.created_at).label("month"),
                extract("year", Walkin.created_at).label("year"),
                counsellor_expr.label("counsellor")
            ).select_from(Walkin)
            .join(Admission, Admission.id == Walkin.admission_id)
        ...
        query = (
                ...
                .group_by(subquery.c.month, subquery.c.year, subquery.c.counsellor)
                .order_by(subquery.c.year, subquery.c.month, subquery.c.counsellor)
            )
        result = await session.execute(query)
    ...
    async def get_due_report(self, from_date: date | None = None,till_date: date | None = None,branch_id: int | None = None,
                             ...
                             student_name: str | None = None,student_phoneno: str | None = None,installment_date: date | None = None,db_session:AsyncSession | None = None):
        session = db_session
        start_date_with_tz = datetime.combine(from_date, datetime.min.time(), pytz.UTC)
        end_date_with_tz = datetime.combine(till_date, datetime.max.time(), pytz.UTC)
        base_filters = []
        ...
        installment_collected = (select(func.count(PurchaseInstallment.id).label("due_rev_installment_count"),
                        ...
                        .where((func.date(Transaction.tx_at).between(start_date_with_tz,end_date_with_tz)),PurchaseInstallment.installment_status ==  "COMPLETED",or_(PurchaseInstallment.is_deleted == False,PurchaseInstallment.is_deleted == None),*base_filters))
    ...
    async def get_fee_follow_up(self,from_date: date | None = None,till_date: date | None = None,branch_id: int | None = None,batch_id:int | None = None,
                                ...
                                offset: int | None = None, student_name: str | None = None,student_phoneno: str | None = None,installment_date: date | None = None):
        session = db_session
        ...
        query = (select(subquery, Purchase.purchase_status.label("purchase_status"), User.full_name.label("student_name"),User.id.label("user_id"),User.email.label("user_email"),User.phone_number.label("user_phno"),
                        ...
                        Product.batch_id.label("batch_id"),Admission.branch_id.label("branch_id"))
                        .select_from(Purchase)
    ...
    async def get_upcoming_installments_by_student_id(self,student_id:int,upto_date:date,db_session:AsyncSession | None = None):
        session = db_session
