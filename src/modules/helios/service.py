...
class HeliosIntegrationService:
	...
	async def resolve_subject_names_to_ids(self, subject_names: list[str]) -> dict[str, int]:
		"""Best-effort mapping from engine macro subjects → CMS subject IDs."""
	...
	async def generate_plan_from_wizard(self, req: GeneratePlanFromWizardRequest) -> int:
		...
		try:
			...
			# Get Haskell adaptive client (HTTP)
			client = await get_helios_adaptive_client()
			...
			sp = await self.studyplan_crud.create(
				...
				object=BEStudyPlanCreate(
					...
					status="DRAFT",
					intake_data=req.wizard,  # Store wizard data for export
				),
			)
	...
	async def get_study_plan_for_export(self, studyplan_id: int) -> tuple[StudyPlanSchema, dict]:
		"""
		...
		Returns:
			Tuple of (StudyPlanSchema object, intake_data dict)
		...
		try:
			...
			# Get intake data from study plan
			intake_data = getattr(study_plan, 'intake_data', None)
			
			return study_plan_schema, intake_data
