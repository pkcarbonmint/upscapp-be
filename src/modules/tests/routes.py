...
from src.users.schemas import FeatureName, QuotaName
...
@test_router.get("/totd", response_model=list[TestResponse])
...
@test_router.get("/pyq", response_model=list[TestResponse])
...
@test_router.get("/model", response_model=list[TestResponse])
...
async def get_performance_benchmark_by_papers(
    *,
    ...
    cms_res = await strapi.get_entries("papers", filters=filters)
...
async def test_attempt_start(
    test_id: int, current_user: User = Depends(CheckUserAccess(subscr=True))
):
    test_db = await test_service.get(id=test_id,db_session=db.session)
    ...
    test_attempt_db = await test_attempt_service.create(
        obj_in=TestAttemptCreate(test_id=test_id, attempted_by_id=current_user.id),db_session=db.session
    )
    ##update test attempt
...
async def test_attempt_submit(
    ...
    if paper_type == PaperType.subjective:
        feature_name = FeatureName.mains
    if not user_db.subscription.free_plan:
