...
async def create_custom_mains_test(test_in: TestCreateMains,is_qbank_create:bool | None = None, user: User = Depends(CheckUserAccess())):
    ...
    if is_qbank_create:
        is_check = await CheckV2UserAccess.check_prod_quota(
    ...
    if is_qbank_create:
        active_pur = await user_service.get_active_purchases_for_student(student_id=user.id,db_session=db.session)
        product = active_pur[0]["product"]
        purchases = active_pur[0]["purchases"]
        quota_name=QuotaName.test_create
        if len(purchases) == 1:
            ...
            for purchase in purchases:
                ...
                quotas= product["product_details"]["q_bank_info"].get("quotas")

                ## iterating to check for each quota name
                if quota_name and quotas:
                    for quota in quotas:
                        if quota.get("quota_name") == quota_name:
                            quota_allowed = quota.get("quota_allowed")
                            break  # Stop searching once found

                if quota_allowed is not None:
...
async def get_custom_tests_by_status(test_in:TestCustomByStatus,current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce,USER_TYPE.student]))):
    tests = await test_attempt_service.get_mains_tests_by_status(user_id=current_user.id,paper_ids=test_in.paper_ids,subject_ids=test_in.subject_ids,topic_ids=test_in.subject_ids,
...
async def get_custom_new_tests(test_in:TestCustomByStatus,current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student]))):
    tests = await test_attempt_service.get_unattempted_customized_users_mains_tests_by_status(user_id=current_user.id,paper_ids=test_in.paper_ids,subject_ids=test_in.subject_ids,topic_ids=test_in.subject_ids,offset=test_in.offset,limit=test_in.limit,db_session=db.session)
    return tests
...
async def test_qs_download(test_id:int):
...
async def test_attempt_start(
    test_id: int,is_qbank_attempt: bool | None = None,plantask_id:int | None = None, current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce,USER_TYPE.student]))
):
    if is_qbank_attempt:
        is_check = await CheckV2UserAccess.check_prod_quota(
            db_session=db.session,
    ...
    if is_qbank_attempt:
        active_pur = await user_service.get_active_purchases_for_student(student_id=current_user.id,db_session=db.session)
        product = active_pur[0]["product"]
        purchases = active_pur[0]["purchases"]
        quota_name=QuotaName.test_attempt if test_db.test_type == TEST_TYPE.custom else test_db.test_type
        if len(purchases) == 1:
...
async def update_test_evaluaton_annotations(test_evaluation_id:int, annotaion_update: TestEvalAnnotationUpdate ):
...
async def get_test_evaluatons(test_evaluation_id:int):
...
async def submit_mains_test(*,id:int,mains_submit_in:MainsSubmitSchema,current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student]))):
    ...
    if test_attempt_db.is_qbank_attempt:
        is_check = await CheckV2UserAccess.check_prod_quota(
            db_session=db.session,
...
async def assign_evaluator(*,test_attempt_id:int,test_assign_in:TestAttemptAssign):
...
async def start_evaluation(*,test_attempt_id:int,eval_start_in:TestAttemptEvalStart):
...
async def assign_reviewer_and_review(*,
    test_attempt_id: int,
    test_review_in: TestReviewSchema):
...
async def withdraw_assigned_test(*,test_attempt_id:int,test_eval_id:int):
...
async def request_reevaluation(*,test_attempt_id:int, reeval_in:TestAttemptReEval):
...
async def get_evaluators_reviewers(workforce_type: str):
...
async def get_attempt_eval_count():
...
async def get_evaluator_evaluations_count(*,evaluator_id:int):
...
async def get_reviewer_review_count(*,reviewer_id:int):
