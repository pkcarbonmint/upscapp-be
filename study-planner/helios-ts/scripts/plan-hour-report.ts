...
export function blockReport(plan: StudyPlan) {
  return plan.cycles
    .flatMap(({cycleType, cycleBlocks }) => {
      return cycleBlocks.flatMap((block) => {
        ...
        return { cycleType, subjects, startDate, endDate,  duration };
      });
    })
    .map(blockInfo => {
      const start = blockInfo.startDate.format('DD-MMM');
      const end = blockInfo.endDate.format('DD-MMM');
      return `${blockInfo.cycleType} ${blockInfo.subjects.join(', ')} ${start} to ${end} ${blockInfo.duration} days`;
    })
    .join('\n');
}