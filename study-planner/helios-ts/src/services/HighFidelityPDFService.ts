...
export class HighFidelityPDFService {
  ...
  static async generateStructuredPDF(
    ...
    try {
      ...
      const htmlContent = this.createStructuredHTML(studyPlan, studentIntake);
  ...
  static async generateVisualPDF(
    ...
    try {
      ...
      const htmlContent = this.createVisualHTML(studyPlan, studentIntake);
   ...
   */
  private static createStructuredHTML(studyPlan: StudyPlan, studentIntake: StudentIntake): string {
    ...
    <body>
        ...
        ${this.generateResourcesSection(studyPlan, uniqueSubjects)}
    </body>
    </html>`;
  }
   ...
   */
  private static createVisualHTML(studyPlan: StudyPlan, studentIntake: StudentIntake): string {
    const totalWeeks = this.calculateTotalWeeks(studyPlan);
    const totalBlocks = this.countTotalBlocks(studyPlan);
    const uniqueSubjects = this.getUniqueSubjects(studyPlan);
  ...
  private static async generateResourcesSection(studyPlan: StudyPlan, uniqueSubjects: string[]): Promise<string> {
    ...
    for (const subjectCode of uniqueSubjects.slice(0, 10)) { // Limit to prevent overwhelming
      try {
        ...
        resourcesHTML += `
        <div class="subject-resources">
            <h4>${this.getSubjectName(subjectCode)}</h4>
