...
// start in the correct target year (2027) when the student targets 2027 exams
...
describe('generateInitialPlan', () => {
  ...
  it('should generate a valid study plan with correct structure', async () => {
    ...
    expect(result.plan.plan_title).toContain('2027');
    expect(result.logs).toBeDefined();
    expect(Array.isArray(result.logs)).toBe(true);
  });

  it('should include foundation cycle for October 2025 start with 2027 target', async () => {
    const result = await generateInitialPlan(
      'test-user-123',
      testConfig,
      testArchetype,
    ...
    if (foundationCycle) {
      ...
      // For October 2025 start with 2027 target, foundation cycle should exist
      // If it doesn't, there might be an issue with the cycle planning logic
      expect(foundationCycle).toBeDefined();
    }
  });

  it('should include prelims revision cycle for 2027 target year', async () => {
    const result = await generateInitialPlan(
      'test-user-123',
      testConfig,
      testArchetype,
    ...
    // Should include prelims revision cycle (Jan 1 - Mar 31, 2027)
    ...
    if (prelimsRevisionCycle) {
      ...
      // Should start around January 2027
      const startDate = dayjs(prelimsRevisionCycle.cycleStartDate);
      expect(startDate.year()).toBe(2027);
      expect(startDate.month()).toBe(0); // January (0-indexed)
    }
  });

  it('should include prelims rapid revision cycle for 2027 target year', async () => {
    const result = await generateInitialPlan(
      'test-user-123',
      testConfig,
      testArchetype,
    ...
    // Should include prelims rapid revision cycle (Apr 1 - May 20, 2027)
    ...
    if (prelimsRapidCycle) {
      ...
      // Should start around April 2027
  ...
  it('should include mains rapid revision cycle for 2027 target year', async () => {
    const result = await generateInitialPlan(
      'test-user-123',
      testConfig,
      testArchetype,
    ...
    // Should include mains rapid revision cycle (May 21 - Aug 20, 2027)
  ...
  it('should have correct target year context', async () => {
    ...
    expect(plan.created_for_target_year).toBe('2027');

    // Should have effective season context
    expect(plan.effective_season_context).toBeDefined();
    // For 2027 target with Oct 2025 start, should be ComprehensiveStudy
            ...
            dailyPlans.forEach(dailyPlan => {
              ...
              if (expectedRatios[cycle.cycleType]) {
                ...
                if (totalTasks > 0) {
                  ...
                  if (cycle.cycleType === CycleType.C4 || cycle.cycleType === CycleType.C5) {
  ...
  it('should ensure blocks do not exceed cycle end dates', async () => {
    ...
    if (plan.cycles) {
      ...
      // Specifically test prelims rapid revision cycle ends on May 20, 2027
      const prelimsEndDate = '2027-05-20';
      const mainsEndDate = '2027-08-20';
  ...
  it('should ensure at least one foundation cycle and one prelims revision cycle are included', async () => {
    ...
    if (plan.cycles && plan.cycles.length > 0) {
      ...
      // For October 2025 start with 2027 target, we should have cycles
      expect(plan.cycles!.length).toBeGreaterThan(0);
    }
  });

  // Test the incremental planning functionality for 2027 target year
  it('should handle incremental planning correctly for 2027 target year', async () => {
    // Test incremental planning from October 2025 to July 2027
    // This should call generateInitialPlan 10 times (Oct 2025, Nov 2025, ..., Jul 2027)
    ...
    incrementalResults.forEach(result => {
      expect(result.plan.created_for_target_year).toBe('2027');
    });

    // Verify that the initial plan has proper timeline utilization
    // The initial plan should cover everything from Oct 2025 to August 2027
    ...
    if (initialResult.plan.cycles) {
      ...
      const foundationCycle = initialResult.plan.cycles.find((cycle: any) =>
        cycle.cycleType === 'FoundationCycle'
      );
      expect(foundationCycle).toBeDefined();

      // Should have prelims revision cycle (Jan 2027 - Mar 2027)
      ...
      // Should have prelims rapid revision cycle (Apr 2027 - May 2027)
      const prelimsRapidCycle = initialResult.plan.cycles.find((cycle: any) =>
        cycle.cycleName.includes('Prelims Rapid')
      );
      expect(prelimsRapidCycle).toBeDefined();

      // Should have mains rapid revision cycle (May 2027 - Aug 2027)
    ...
    // For 2027 target year starting Oct 2025, students will have comprehensive study throughout
    // The first 9 plans (Oct 2025 through Jun 2027) should be ComprehensiveStudy
    // The last plan (Jul 2027) would be ExclusiveMains if we were in July 2027
