...
import { ConfidenceLevel, ConfidenceMap, CycleSchedule, CycleType, DailyHourLimitsInput, DayOfWeek, DayPreferences, determineBlockSchedule, determineCycleSchedule } from 'scheduler';
...
function ignore_it(description: string, fn: () => void) {
...
describe('CycleSchedulerScenarios', () => {
  ...
  describe('Block Tests', () => {
    ignore_it('should generate a block schedule for all cycles in T1', () => {
      const subjects = SubjectLoader.loadAllSubjects();
      const intake = makeT1Intake();
      const {schedules} = determineCycleSchedule(dayjs("2025-03-02").toDate(), 2027, dayjs("2027-05-28").toDate(), intake);
      const report = schedules.map((cycleSchedule) => {
        ...
        const endDates = blockSchedules.map((block) => ({cycle: cycleSchedule.cycleType, subject: block.subjectCode, blockEndDate: block.endDate }));
        return endDates;
      }).flat()
      .map(({cycle, subject, blockEndDate}: {cycle: CycleType, subject: string, blockEndDate: string}) => `[${cycle.toString()}] ${subject}: ${blockEndDate}`)
      .join("\n");
      console.log('**--'.repeat(20)+'\n', report);

      for (const cycleSchedule of schedules) {
        const workingHoursPerDay = intake.getDailyStudyHours();
        const totalHours = dayjs("2025-05-31").diff(dayjs("2025-03-01"), "day") * workingHoursPerDay;
        const gsOptionalRatio = intake.getGSOptionalRatio ? intake.getGSOptionalRatio() : { gs: 0.67, optional: 0.33 };
        const studyApproach = convertToSchedulerStudyApproach(intake.subject_approach);
...
console.log("-~*".repeat(20));
      ...
      const { blockSchedules } = result;
      console.log('Result:', blockSchedules.map(block => ([block.subjectCode,block.allocatedHours, block.startDate,block.endDate])));
      console.log('total allocated hours in blocks: ', blockSchedules.reduce((sum, block) => sum + block.allocatedHours, 0));
