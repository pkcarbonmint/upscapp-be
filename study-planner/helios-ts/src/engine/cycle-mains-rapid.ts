...
export async function planMainsRevisionCycle(
  ...
  // If no start date, cycle is not included
  if (!startDate) {
    info('Engine', 'Mains revision cycle skipped: no valid start date');
    return undefined;
  }
  ...
  const targetYear = parseInt(intake.target_year);
  const endDate = dayjs(`${targetYear}-07-31`);
...
/**
 * Plan Mains Rapid Revision cycle: May 21 of target year until August 20 of target year
 * - Apply confidence factors to baseline hours
 * - Trim subtopics from D5 down to B1 if needed
 * - Returns undefined if no time for this cycle
 */
export async function planMainsRapidRevisionCycle(
  ...
  const { logInfo: info, logDebug: debug } = logger;
  const { subjects } = subjData;
  info('Engine', 'Planning Mains Rapid Revision cycle');

  const durationWeeks = Math.ceil(endDate.diff(startDate, 'day') / 7);

  // Calculate total hours for mains rapid revision cycle (assuming 8 hours/day)
  const totalHours = durationWeeks * 7 * 8;

  debug('Engine', `Mains rapid revision cycle: ${durationWeeks} weeks, ${totalHours} total hours`);
  ...
  const blocks = await createBlocksForSubjects(
    ...
    totalHours,
    confidenceMap,
    'Mains Rapid Revision', 'C7',
    4,
    'Mains Rapid Revision Cycle',
    startDate.format('YYYY-MM-DD'),
    endDate.format('YYYY-MM-DD'),
    subjData);

  const cycle: StudyCycle = {
    cycleId: `mains-rapid-revision-${Date.now()}`,
    cycleType: 'C7',
    cycleIntensity: 'Intensive',
    cycleDuration: durationWeeks,
    cycleStartWeek: 1,
  ...
  info('Engine', `Created Mains rapid revision cycle with ${cycle.cycleBlocks.length} blocks`);
