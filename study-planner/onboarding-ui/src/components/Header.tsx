...
import { CrossAppNavigation, useSharedAuth } from 'shared-ui-library';
...
const Header: React.FC<HeaderProps> = ({ userName }) => {
  ...
  return (
    <>
      ...
      {/* Cross-app navigation - only show if authenticated with faculty access */}
      {isAuthenticated && user?.user_type === 'faculty' && (
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2">
          <CrossAppNavigation currentApp="onboarding" />
        </div>
      )}
    </>
  );
