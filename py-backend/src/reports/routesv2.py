...
from src.reports.schemas import AggMainsReportIn, BatchSpecificReport, PrelimsReportIn, ReportIn,TestSpecificReportIn
...
from .services import agg_mains_paper_analysis, agg_mains_perf_summmary, agg_mains_score_bnmark, agg_mains_subj_analysis, agg_mains_test_scores, agg_mains_tests_analysis, agg_paper_mains_trend, agg_pef_summary,agg_overall, agg_prelims_accuracy_bnmark, agg_prelims_overall, agg_prelims_pef_summary, agg_prelims_score_bnmark, agg_prelims_subj_analysis, agg_prelims_technique, agg_prelims_topic_analysis, agg_prelims_trend, agg_subj_mains_trend,agg_trend,agg_score_bnmark,agg_accuracy_bnmark,agg_technique,agg_subj_analysis,agg_topic_analysis, aggr_mains_paper_perf, check_user_mains_testattempt, test_prelims_topic_analysis,test_topic_analysis
...
@report_router_v2.post("/aggregate/prelims")
async def get_agg_report(report_in:PrelimsReportIn, current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student]))):
    
    user = current_user
    user_tests = await test_attempt_service.calc_prelims_ta_count(user_id=current_user.id,stage_ids=report_in.stage_ids,from_date=report_in.from_date,till_date=report_in.till_date,db_session=db.session)
    fl_user_tests = await test_attempt_service.calc_prelims_ta_count_of_fl(user_id=current_user.id,stage_ids=report_in.stage_ids,from_date=report_in.from_date,till_date=report_in.till_date,is_full_length=True,db_session=db.session)
    
    if user_tests >0 or fl_user_tests >2:
        # Aggregate Full length 
        aggr_pef_summary = await agg_prelims_pef_summary(current_user = user ,from_date=report_in.from_date,till_date=report_in.till_date, stage_ids=report_in.stage_ids,db_session=db.session)
        aggr_overall = await agg_prelims_overall(current_user=user,stage_ids=report_in.stage_ids,from_date=report_in.from_date,till_date=report_in.till_date,db_session=db.session)
        aggr_trend = await agg_prelims_trend(current_user=user,stage_ids=report_in.stage_ids,from_date=report_in.from_date,till_date=report_in.till_date,full_length_result=True,db_session=db.session)
        aggr_score_bnmark = await agg_prelims_score_bnmark(current_user=user,stage_ids=report_in.stage_ids,from_date=report_in.from_date,till_date=report_in.till_date,is_full_length=True,db_session=db.session)
        aggr_accuracy_bnmark = await agg_prelims_accuracy_bnmark(current_user=user,stage_ids=report_in.stage_ids,from_date=report_in.from_date,till_date=report_in.till_date,is_full_length=True,db_session=db.session)
        aggr_technique = await agg_prelims_technique(current_user=user,stage_ids=report_in.stage_ids,from_date=report_in.from_date,till_date=report_in.till_date,is_full_length=True,db_session=db.session)
        aggr_subj_analysis = await agg_prelims_subj_analysis(current_user=user,stage_ids=report_in.stage_ids,is_full_length=True,from_date=report_in.from_date,till_date=report_in.till_date,db_session=db.session)
        aggr_topic_analysis = await agg_prelims_topic_analysis(current_user=user,stage_ids=report_in.stage_ids,is_full_length=True,from_date=report_in.from_date,till_date=report_in.till_date,db_session=db.session)
        ...
        # Aggregate Q Bank
        aggr_q_bank_technique = await agg_prelims_technique(current_user=user,stage_ids=report_in.stage_ids,from_date=report_in.from_date,till_date=report_in.till_date,is_full_length=False,db_session=db.session)
        aggr_q_bank_subj_analysis = await agg_prelims_subj_analysis(current_user=user,stage_ids=report_in.stage_ids,from_date=report_in.from_date,till_date=report_in.till_date,is_full_length=False,db_session=db.session)
        aggr_q_bank_topic_analysis = await agg_prelims_topic_analysis(current_user=user,stage_ids=report_in.stage_ids,from_date=report_in.from_date,till_date=report_in.till_date,is_full_length=False,db_session=db.session)
        ...
        "subj_wise_resp":aggr_subj_analysis,"topic_wise_resp": await test_prelims_topic_analysis(topic_analysis=aggr_topic_analysis,stage_ids=report_in.stage_ids),
        ...
        "qb_topic_wise_resp": await test_prelims_topic_analysis(topic_analysis=aggr_q_bank_topic_analysis,stage_ids=report_in.stage_ids)
        }
...
async def get_agg_report(report_in:AggMainsReportIn, current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student]))):
...
@report_router_v2.post("/tests/specific/prelims")
async def get_test_specific(report_in:TestSpecificReportIn, current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student]))):
...
async def get_batch_specific(report_in:BatchSpecificReport, current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student]))):
