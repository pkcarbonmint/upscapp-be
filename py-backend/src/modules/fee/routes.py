...
async def get_fee_by_date(*,from_date: date,
        till_date: date, branch_id:int | None = None,filter_dropout:bool | None = None, offering_id:int | None = None,batch_id:int| None = None,user:User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce],roles=[USER_ROLE.director,USER_ROLE.finance_report_user,USER_ROLE.finance_manager],apps=[APP.dashboard_app]))):
        
     amounts = await purchase_service.get_all_fees(from_date=from_date,till_date=till_date,branch_id=branch_id,offering_id=offering_id,batch_id=batch_id,filter_dropout=filter_dropout,db_session=db.session)
     return amounts
...
async def get_day_report(report_in:DayReportSchema,user:User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce],roles=[USER_ROLE.director,USER_ROLE.collection_done_report_user,USER_ROLE.finance_manager],apps=[APP.dashboard_app]))):
     reports = await purchase_service.get_day_report(from_date=report_in.from_date,till_date=report_in.till_date,
                                                     branch_ids=report_in.branch_ids,offering_id=report_in.offering_id,tx_id=report_in.tx_id,
                                                     batch_id=report_in.batch_id,legal_entity=report_in.legal_entity,payment_mode=report_in.payment_mode,include_incomplete_txs=report_in.include_incomplete_txs,is_online_branch=report_in.is_online_branch,plan_name=report_in.plan_name,limit = report_in.limit,offset = report_in.offset,db_session=db.session)
...
async def get_uncollected_amt(*,
        ...
        filter_dropouts: bool | None = None,user:User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce],roles=[USER_ROLE.director,USER_ROLE.finance_manager,USER_ROLE.finance_report_user],apps=[APP.dashboard_app]))):
     ...
     uncollected_amount = await purchase_service.get_uncollected_amt(batch_id=batch_id,branch_id= branch_id,offering_id=offering_id,from_date=from_date,till_date=till_date,filter_dropouts=filter_dropouts,db_session=db.session)
...
async def get_due_report(due_count:DueCountSchema,user:User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce],roles=[USER_ROLE.director,USER_ROLE.fee_collection_incharge],apps=[APP.dashboard_app,APP.front_desk_app]))):
     count = await purchase_service.get_due_report(from_date=due_count.from_date,till_date=due_count.till_date,branch_id=due_count.branch_id,
                                                   ...
                                                   student_name=due_count.student_name,student_phoneno=due_count.student_phoneno,
                                                     installment_date=due_count.installment_date,filter_dropouts=due_count.filter_dropouts,db_session=db.session)
     return count
...
async def get_fee_followup(*,fee_followup:FeeFollowupSchema, user:User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce],roles=[USER_ROLE.director,USER_ROLE.due_report_user,USER_ROLE.finance_manager, USER_ROLE.fee_collection_incharge],apps=[APP.dashboard_app,APP.front_desk_app]))):
     fees = await purchase_service.get_fee_follow_up(from_date=fee_followup.from_date,till_date=fee_followup.till_date,legal_entity=fee_followup.legal_entity,
                                                     ...
                                                     installment_date=fee_followup.installment_date,filter_dropouts=fee_followup.filter_dropouts)
     
     return fees

@fee_router.get("/student/upcoming/installments")
async def get_student_upcoming_installments(student_id:int,upto_date:date,filter_dropouts: bool | None = None, user: User = Depends(CheckV2UserAccess())):
     upcoming = await purchase_service.get_upcoming_installments_by_student_id(student_id=student_id,upto_date=upto_date,filter_dropouts=filter_dropouts,db_session = db.session)
     return upcoming

@fee_router.get("/offerings/batches/count")
async def get_offering_batches_admission_count(offering_id:int| None = None, user: User = Depends(CheckV2UserAccess())):
     count_resp = await purchase_service.get_offering_batches_admission_count(offering_id=offering_id,db_session=db.session)
     return count_resp

@fee_router.get("/products/purchases/count")
async def get_product_purchases_count(user: User = Depends(CheckV2UserAccess())):
     count_resp = await purchase_service.get_product_purchases_count(db_session=db.session)
     return count_resp