...
class Product(Base,BaseMixin):
    ...
    target_count = mapped_column(Integer)
...
class Enrollment(Base,BaseMixin):
    ...
    # __table_args__ = (UniqueConstraint("product_id", "admission_id", "enrolled_user_id"),)
...
class Price(Base,BaseMixin):
    ...
    session_break_up = mapped_column(ARRAY(JSON))
...
class Purchase(Base,BaseMixin):
    ...
    admission_id = mapped_column(ForeignKey("admissions.id"), nullable = True)
    walkin_id = mapped_column(ForeignKey("walkins.id"),nullable= True)
    ...
    product_sessions = mapped_column(JSON)
