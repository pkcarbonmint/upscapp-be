...
from src.users.service import SubscriptionService
...
from src.users.models import Subscription, User, Transaction
...
async def create_batch(*, batch: BatchCreate, user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce], roles=[USER_ROLE.org_admin, USER_ROLE.branch_admin, USER_ROLE.product_admin],apps=[APP.admin_app]))):
    ...
    #     product_db:ProductResponse = await product_crud.get_by_field(value=offering_db.id,field="offering_id", db=db.session)
    #     admin_branch_ids = await user_role_service.get_branches_for_role_user(role=USER_ROLE.branch_admin,user_id=user.id,db_session=db.session)
...
async def update_price(*, id:int, price: PriceUpdate, user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce], roles=[USER_ROLE.org_admin, USER_ROLE.branch_admin, USER_ROLE.product_admin],apps=[APP.admin_app]))):
    ...
    if price.session_break_up:
        rate = price.selling_price or price_db.selling_price
        total = sum(p.price for p in price.session_break_up)
        if rate != total:
            return ResponseSchema(msg="Sum of sessions not equal to selling price")
...
async def get_products_for_enrollment(*, enrollment_products_in: EnrollmentProduct,current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student])) ):
...
async def get_student_with_guide_mentor(*, product_id:int,batch_id:int | None = None,name:str | None = None,student_id:int | None = None,email:str | None = None,phone_no:str | None = None, offset: int = 0,limit: int = 100,current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student]))):
...
async def get_students_by_teacher(*,teacher_id:int,
                                    ...
                                    limit: int | None = None,
                                    offset: int | None = None,current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student]))):
...
async def get_enrollment_summary(
    user_id: int,current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student]))
...
async def get_students_count_for_prod_batch(*, filters: Any | None = None,current_user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce, USER_TYPE.student]))):
...
async def create_purchases(*, purchases_create: FDPurchaseSchema, user:User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce,USER_TYPE.student],roles=[USER_ROLE.admission_manager,USER_ROLE.student_administrator],apps=[APP.front_desk_app,APP.student_app]))):
    ...
    for purchase in purchases_create.purchases:
        ...
        purchase_create = PurchaseCreate(product_id=product_db.id,
                                         ...
                                         product_sessions= purchase.product_sessions
...
async def purchase_pay_offline(*,payment_in:PaymentOfflineSchema, request: Request,
                               user:User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce,USER_TYPE.student],roles=[USER_ROLE.admission_manager, USER_ROLE.student_administrator,USER_ROLE.fee_collection_incharge],apps=[APP.front_desk_app]))):
    async def check_for_tx(tx_id: int, pur_ids: list[int] | None = None, ins_ids: list[int] | None = None):
        ...
        if tx_db.payment_mode == "PAYMENT_LINK":
            ...
            print("LINKS_STSO>>>",link_status_data)
            if link_status_data and link_status_data.get("status") == "created":
                tx_ids = []
                if pur_ids:
                    for id in pur_ids:
...
@purchase_router.post("/purchases/admission/enrollments",response_model=ResponseSchema[AdmissionEnrollmentPurResp])
async def create_admission_enrollment(*,data:AdmissionEnrollmentCreate):
    purchase_in_db = await purchase_crud.get(id = data.purchase_id, db=db.session)
    if purchase_in_db.admission_id:
        return ResponseSchema(msg="Purchase already has admission")
    admission_in = AdmissionCreate(**data.model_dump(exclude_unset=True),status="NEW",user_id=data.student_id, admission_details={
                "admissionManagerName": data.manager_name,
                "admissionManagerId": data.manager_id
            })
    admission:Admission = await admission_crud.create(object=admission_in,db=db.session)
    product:Product = await product_crud.get(id=data.product_id,db=db.session)
    enrollment_in = EnrollmentCreate(**data.model_dump(exclude_unset=True),
                                     admission_id=admission.id,
                                     batch_id=product.batch_id,enrollment_status= "ACTIVE",
                                     enrolled_as="STUDENT",enrolled_user_id=data.student_id,
                                     offering_id = product.offering_id)
    enrollment: Enrollment = await enrollment_crud.create(object=enrollment_in, db=db.session)
    purchase_update_db = await purchase_crud.update(object={"admission_id":admission.id},id = data.purchase_id,db=db.session)
    purchase_db = await purchase_crud.get(id = data.purchase_id, db=db.session)
    return ResponseSchema(data={"admission":admission,"enrollment":enrollment,"purchase":purchase_db},success=True)
...
async def get_tx_by_date_filter(*,report_filters:TxReport,user:User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce],roles=[USER_ROLE.admission_manager,USER_ROLE.student_administrator],apps=[APP.front_desk_app]))):
    tsx = await user_service.txs_by_date_window(start_date=report_filters.start_date,end_date=report_filters.end_date,
                                                branch_ids=report_filters.branch_ids,is_online_branch= report_filters.is_online_branch,tx_id = report_filters.tx_id,
                                                include_incomplete_txs = report_filters.include_incomplete_txs,plan_name = report_filters.plan_name,limit=report_filters.limit,offset=report_filters.offset,db_session=db.session)
...
async def get_pending_purchases(*, report_filters:PurchaseReport,user:User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce],roles=[USER_ROLE.admission_manager,USER_ROLE.student_administrator],apps=[APP.front_desk_app]))):
    txs = await user_service.pending_purchases(branch_ids=report_filters.branch_ids,filter_dropout=report_filters.filter_dropout,limit=report_filters.limit,offset=report_filters.offset,db_session=db.session)
...
async def get_payment_receipt(*,id:int, user:User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce,USER_TYPE.student]))):
    ...
    data = {"laex_icon":"src/assets/la-excellence-logo.svg","receipt_id":tx.id,"date": format_date(date_str=tx.tx_at),"tx":tx, "tx_created": format_date_with_time(date_input=tx.created_at),
...
async def get_product_recommendations(*,product_ids:list[int], user_id:int, user: User = Depends(CheckV2UserAccess())):
...
async def get_invoice_receipt(*,data_in:InvoiceSchema, user: User = Depends(CheckV2UserAccess())):
    tx_data = []
    def format_date(date_str):
        if isinstance(date_str, str):
            date_str = datetime.fromisoformat(date_str)
    ...
    for tx_id in data_in.tx_ids:
            ...
            if purchase_installments:                
                ...
                if purchases:
                    ...
                    print(f"No purchases with receipt {tx.id}")
                    user = await user_service.get(id=tx.paid_by, db_session=db.session)
                    subs = await subscription_service.get_by_field(value=tx.subscription_id,field="id", db_session=db.session)
                    if not subs:
                        continue
                    else:
                        total_base_amt = tx.amount - ((18/100)*tx.amount)
                        total_gst_amt  = (18/100)*tx.amount
                        total_price_amt = tx.amount
            ...
            data = {
                ...
                "total_sub_gst_amt":total_gst_amt,
            ...
            if not subs:
                ...
                template = env.get_template('invoice_sub.html')
            output = template.render(data)
            pdf_fileobj = HTML(string=output, base_url='.').write_pdf()
