...
class Walkin(Base, BaseMixin):
    ...
    admission_id = mapped_column(ForeignKey("admissions.id")) #not using this mapping
    admissions = relationship(
        "Admission",
        back_populates="walkin",
        foreign_keys="Admission.walkin_id",   # tell SQLAlchemy to use this path
        lazy="joined"
    )
