...
import requests
from sqlalchemy import Date, cast, func, text, update
from src.base.schemas import ResponseSchema,ResponseListSchema,ResponsePaginatedSchema
from src.base.service import BaseCRUD, BaseService
...
from src.modules.products.models import Enrollment, Purchase
...
@frontdesk_router.get("/walkins", response_model=ResponseListSchema[WalkinCountResp])
async def get_walkins_with_filters(*, filters: Any | None = None,limit:int|None = None, offset:int | None = None,start_date:date| None = None, end_date:date|None = None,status: str | None = None,
                                   user_name: str| None = None, phone_number: str| None = None,email: str| None = None,is_closed: bool | None = None,
                                   branch_name: str | None = None, offering_category: str | None = None, offering: str | None = None, counselling_done: bool | None = None,
                                   user: User = Depends(CheckV2UserAccess(user_types=[USER_TYPE.workforce,USER_TYPE.student],roles=[USER_ROLE.front_desk_executive,USER_ROLE.admission_manager,USER_ROLE.admission_counsellor,USER_ROLE.student_administrator,USER_ROLE.admission_counsellor,USER_ROLE.student_administrator,
    ...
    if start_date and end_date and not (counselling_done or is_closed):
            start_utc, end_utc = await ist_range_to_utc(start_date, end_date)
    ...
    if branch_name:
        clause.append(
            cast(Walkin.questionnaire.op("->>")("joining_branch"), String) == branch_name
        )
    if offering_category:
        clause.append(
            cast(Walkin.profile_details.op("->>")("interested_offering_category"), String) == offering_category
        )
    if offering:
        clause.append(
            cast(Walkin.profile_details.op("->>")("interested_offering"), String).ilike(f"%{offering}%")
        )
        
    if counselling_done:
        clause.append(
            (Walkin.counselling.isnot(None)) &
            (func.json_typeof(Walkin.counselling) != "null")
        )
        if start_date and end_date:
            start_utc, end_utc = await ist_range_to_utc(start_date, end_date)
            clause.append(cast(Walkin.counselling.op("->>")("counselling_date"), Date).between(start_utc, end_utc))
    if is_closed:
        if start_date and end_date:
            start_utc, end_utc = await ist_range_to_utc(start_date, end_date)
            clause.append(Walkin.updated_at.between(start_utc, end_utc))

    if user.user_type == USER_TYPE.student:
        walkins = await walkin_service.get_walkins_with_count(db=db.session, filters=filter_data,clause_statements=clause if clause else None, attr="user_id", ids=[user.id],skip=offset,limit=limit)
        # walkin_count = await walkin_crud.get_count_by_filters(db=db.session, filters=filter_data)
    else:
        walkins = await walkin_service.get_walkins_with_count(db=db.session, filters=filter_data,clause_statements=clause if clause else None,skip=offset,limit=limit)
...
@frontdesk_router.put("/admissions/{id}/dropout", response_model=ResponseSchema[AdmissionResponse])
async def update_admission(*, id:int, dropout:bool,dropout_reason: str | None = None, user: User = Depends(CheckV2UserAccess())):
    admission_db = await admission_crud.get(db=db.session,id=id)
    if dropout is True:
        admission_update_db = await admission_crud.update(db=db.session,id=id,object={"is_dropout":dropout, "dropout_reason": dropout_reason if dropout_reason else admission_db.dropout_reason})
        ad_enrolls = await enrollment_crud.get_by_filters_multi(db=db.session,filters={"admission_id":id})
        for enroll in ad_enrolls:
            await enrollment_crud.update(db.session,{'enrollment_status': "INACTIVE"},id=enroll.id) 
    else:
        admission_update_db = await admission_crud.update(db=db.session,id=id,object={"is_dropout":dropout, "dropout_reason": dropout_reason if dropout_reason else admission_db.dropout_reason})
        ad_enrolls = await enrollment_crud.get_by_filters_multi(db=db.session,filters={"admission_id":id})
        for enroll in ad_enrolls:
            await enrollment_crud.update(db.session,{'enrollment_status': "ACTIVE"},id=enroll.id) 
...
async def get_walkin_form(*, walkin_id:int, walkin_type: WALKIN_TYPE, user: User = Depends(CheckV2UserAccess())):
    ...
    if walkin_category and walkin_category == WALKINFORM_TYPE.interview_guidance_program:
        ...
        # 3. Add pages from the generated PDF
        ...
        # 4. Write to memory
        merged_pdf_stream = BytesIO() #Creates an in-memory binary buffer (like a file, but in RAM).
        writer.write(merged_pdf_stream) #This writes the final merged PDF (all pages you added earlier) into the BytesIO buffer.
        merged_pdf_stream.seek(0) #Moves the buffer’s read/write cursor back to the beginning.
...
async def get_admission_form(*,admission_id:int, user: User = Depends(CheckV2UserAccess())):
    ...
    walkin = await walkin_crud.get_by_field(value=admission.walkin_id,field="id", db=db.session)
    # walkin = admission.walkin_id
    if not walkin:
...
@frontdesk_router.post("/walkin/counsellings")
async def get_counselling_details(counselling_det: CounsellingDetails, user: User = Depends(CheckV2UserAccess())):
    walkins = await walkin_service.get_counselling_details(counselling_date=counselling_det.counselling_date,
                                                           counsellor_name=counselling_det.counsellor_name,
                                                           from_date= counselling_det.from_date,
                                                            till_date=counselling_det.till_date,
                                                           offset=counselling_det.offset,
                                                           limit=counselling_det.limit,db_session=db.session)
    return walkins

@frontdesk_router.post("/admissions/details", response_model=list[AdmissionReportDetails])
async def get_admission_details(admission_det: AdmissionDet, user: User = Depends(CheckV2UserAccess())):
     admissions = await walkin_service.get_admission_summary(user_id=admission_det.user_id,
                                                             student_name=admission_det.student_name,
                                                             ph_no=admission_det.ph_no,
                                                             branch_name=admission_det.branch_name,
                                                             offering_category=admission_det.offering_category,
                                                             offering_name=admission_det.offering_name,
                                                             prod_name=admission_det.prod_name,
                                                             prod_code=admission_det.prod_code,
                                                             admission_id=admission_det.admission_id,
                                                             from_date= admission_det.from_date,
                                                            till_date=admission_det.till_date,
                                                             limit=admission_det.limit,
                                                             offset=admission_det.offset,
                                                             db_session=db.session)
     return admissions

@frontdesk_router.post("/admissions/unrealized")
async def get_admissions_unrealized(admission_det: AdmissionInfo, user: User = Depends(CheckV2UserAccess())):
     admissions = await walkin_service.get_to_be_realized(user_id=admission_det.user_id,
                                                             student_name=admission_det.student_name,
                                                             ph_no=admission_det.ph_no,
                                                             branch_name=admission_det.branch_name,
                                                             offering_category=admission_det.offering_category,
                                                             ...
                                                             prod_code=admission_det.prod_code,
                                                             admission_id=admission_det.admission_id,
                                                             filter_dropout=admission_det.filter_dropout,
                                                             from_date= admission_det.from_date,
                                                             till_date=admission_det.till_date,
                                                             limit=admission_det.limit,
                                                             offset=admission_det.offset,
...
@frontdesk_router.post("/offering/referrers")
async def get_offering_referrers(user: User = Depends(CheckV2UserAccess())):
     count = await walkin_service.get_offering_referrers(db=db.session)

     return count

@frontdesk_router.post("/counsellor/conversions")
async def counsellor_conversion_ratio(data_in:ConversionData, user: User = Depends(CheckV2UserAccess())):
     count = await walkin_service.counsellor_conversion_ratio(from_date= data_in.from_date,
                                                             till_date=data_in.till_date,
                                                             offering_name=data_in.offering_name,
                                                             counsellor_name=data_in.counsellor_name,
                                                             db=db.session)

     return count

@frontdesk_router.post("/counsellor/performances")
async def counsellor_performances(user: User = Depends(CheckV2UserAccess())):
     count = await walkin_service.counsellor_performance(db=db.session)

     return count

@frontdesk_router.post("/counsellor/prediction/calibration")
async def counsellor_prediction_calibration(counsellor_data: CounsellorData, user: User = Depends(CheckV2UserAccess())):
     count = await walkin_service.counsellor_prediction_calibration(from_date=counsellor_data.from_date,
                                                                    till_date=counsellor_data.till_date,
                                                                    counsellor_id=counsellor_data.counsellor_id,db=db.session)
