...
async def razorpay_webhook(
    ...
    if data["event"] == "payment_link.paid":
        ...
        tx_update_db = await transaction_service.update(obj_current=tx_db,obj_new=tx_update,db_session=db.session) 
        log_res = await log_event(db=db.session,request=request,event_type=EVENT_TYPE.ONLINE_PG_PAYMENT_SUCCESS,event_details={"tx_id":tx_db.id,"paid_by":tx_db.paid_by,"purchase_ids":purchase_ids,"installment_ids":purchase_installment_ids})      
        ...
        if hash_link:
            ...
            if link_id:
                ...
                if tx_db:
                    ...
                    tx_update_db = await transaction_service.update(obj_current=tx_db,obj_new=tx_update,db_session=db.session) 
                    await log_event(db=db.session,request=request,event_type=EVENT_TYPE.ONLINE_PG_PAYMENT_FAILURE,event_details={"tx_id":tx_db.id,"paid_by":tx_db.paid_by,"purchase_ids":purchase_ids,"installment_ids":purchase_installment_ids})             
