...
async def get_exams():
    response = await strapi.get_entries("exams", get_all=True)
...
async def get_stages(exam_id: int):
    ...
    response = await strapi.get_entries("stages", filters=filters, get_all=True)
    return process_data(entry=response)
...
async def get_stages():
    # filters = {"exam": {"id": {"$eq": exam_id}}}
    response = await strapi.get_entries("stages", get_all=True)
    return process_data(entry=response)
...
async def get_papers(exam_id: int, stage_id: int):
    ...
    response = await strapi.get_entries("papers", filters=filters, get_all=True)
    return process_data(entry=response)
...
async def get_optional_languages():
    response = await strapi.get_entries(
        ...
        filters={"isLanguage": {"$eq": "true"}, "isOptionalSubject": {"$eq": "true"}}, get_all=True
    )
    return process_data(entry=response)
...
async def get_optional_subjects():
    response = await strapi.get_entries(
        ...
        filters={"isLanguage": {"$eq": "false"}, "isOptionalSubject": {"$eq": "true"}}, get_all=True
    )
...
async def get_all_subjects():
    response = await strapi.get_entries(
        "subjects", get_all=True, sort=['id:asc'], publication_state="preview"
    )
    return process_data(entry=response)
...
async def get_ca_topics(tenant_id: int, exam_id: int, paper_id: int,category: CATEGORY | None = None, is_external: bool | None = None):
    resp = await strapi.get_entries(
        "current-affairs-topics", get_all=True
    )
    topic_resp = process_data(entry=resp)
...
@cms_router_v2.get("/currentaffairs/topics/all", dependencies=[Depends(CheckV2UserAccess())])
async def get_ca_topics():
    resp = await strapi.get_entries(
        "current-affairs-topics", get_all=True
    )
    topic_resp = process_data(entry=resp)
